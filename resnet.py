...
def batch_norm_relu(inputs, is_training, relu=True, init_zero=False,
  ...
  if global_bn:
    ...
    inputs = tf.keras.layers.batch_normalization(
        inputs=inputs,
        axis=axis,
        momentum=batch_norm_decay,
        epsilon=BATCH_NORM_EPSILON,
...
def sk_conv2d(inputs, filters, strides, sk_ratio, min_dim=32,
  ...
  global_features = tf.keras.layers.Conv2D(filters=mid_dim, kernel_size=1, strides=1,
      kernel_initializer=tf.variance_scaling_initializer(),
      use_bias=False, data_format=data_format)(global_features)
  global_features = batch_norm_relu(
      global_features, is_training, data_format=data_format)
  mixing = tf.keras.layers.Conv2D(filters=2 * filters, kernel_size=1, strides=1,
      kernel_initializer=tf.variance_scaling_initializer(),
      use_bias=False, data_format=data_format)(global_features)
...
def se_layer(inputs, filters, se_ratio, data_format='channels_last'):
  ...
  if se_ratio <= 0:
    return inputs
  se_reduce = tf.keras.layers.Conv2D(
      ...
      data_format=data_format,
      use_bias=True)
  se_expand = tf.keras.layers.Conv2D(
      inputs.shape[-1],
      kernel_size=[1, 1],
      strides=[1, 1],
      kernel_initializer=tf.variance_scaling_initializer(),
...
def residual_block(inputs, filters, is_training, strides,
  ...
  if use_projection:
    ...
    if sk_ratio > 0:  # Use ResNet-D (https://arxiv.org/abs/1812.01187)
      ...
      inputs = tf.keras.layers.AveragePooling2D(pool_size=2, strides=strides,
          padding='SAME' if strides == 1 else 'VALID', data_format=data_format)(inputs)
  ...
  if use_projection:
    ...
    if sk_ratio > 0:  # Use ResNet-D (https://arxiv.org/abs/1812.01187)
      if strides > 1:
        ...
        shortcut = inputs
      shortcut = tf.keras.layers.AveragePooling2D(pool_size=2, strides=strides,
          padding='SAME' if strides == 1 else 'VALID', data_format=data_format)(shortcut)
      shortcut = conv2d_fixed_padding(
...
def resnet_v1_generator(block_fn, layers, width_multiplier,
  ...
  def model(initial_inputs, is_training):
    """Creation of the model graph."""
    if cifar_stem:
      inputs = conv2d_fixed_padding(
          inputs=initial_inputs, filters=64 * width_multiplier, kernel_size=3,
      ...
      if sk_ratio > 0:  # Use ResNet-D (https://arxiv.org/abs/1812.01187)
        inputs = conv2d_fixed_padding(
            inputs=initial_inputs, filters=64 * width_multiplier // 2, kernel_size=3,
        ...
        inputs = conv2d_fixed_padding(
            inputs=initial_inputs, filters=64 * width_multiplier, kernel_size=7,
            strides=2, data_format=data_format)
      inputs = tf.identity(inputs, 'initial_conv')
      inputs = batch_norm_relu(inputs, is_training, data_format=data_format)

      inputs = tf.keras.layers.MaxPooling2D(pool_size=3, strides=2, padding='SAME',
          data_format=data_format)(inputs)
      inputs = tf.identity(inputs, 'initial_max_pool')
    ...
    return Model(initial_inputs, inputs)
